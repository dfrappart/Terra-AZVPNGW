...
module "AllowAllFromvNet2toBEAppsIn" {
  #Module source
  ...
  NSGReference                      = "${module.NSG_BEApps_Subnet_vNet1.Name}"
  NSGRuleName                       = "AllowAllFromvNet2toBEAppsIn"
  NSGRulePriority                   = 101
  ...
  NSGRuleDestinationAddressPrefixes = ["${lookup(var.vNet1SubnetAddressRange, 1)}"]
}

module "AllowAllFromBEAppstovNet2Out" {
  #Module source
  source = "./Modules/01 NSGRule_Multiplerange"
  ...
  RGName                            = "${module.ResourceGroup.Name}"
  NSGReference                      = "${module.NSG_BEApps_Subnet_vNet1.Name}"
  NSGRuleName                       = "AllowAllFromBEAppstovNet2Out"
  NSGRulePriority                   = 103
  ...
  NSGRuleSourcePortRanges           = ["*"]
  NSGRuleDestinationPortRanges      = ["*"]
  NSGRuleSourceAddressPrefixes      = ["${lookup(var.vNet1SubnetAddressRange, 1)}"]
